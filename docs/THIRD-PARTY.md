...
`1.14.0-extended-2.7.1`. Основана на
...
которых нет в основной ветке. Мы её не собираем и не правим: берётся готовый
двоичный файл из выпусков форка.

**GNU GPL v3 или новее.** Текст — в `engines/sing-box/LICENSE`, он едет вместе
с движком. Лицензия требует назвать источник изменённой сборки, а не только
исходный проект, поэтому ссылка на выпуски форка приводится рядом с этим
файлом при каждом выпуске.
