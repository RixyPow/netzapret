    ...
    private static int PrintUsage(int exitCode)
    {
        Console.WriteLine("""
            ...
            blockcheck
              ...
              --out <файл>         Записать отчёт ещё и в файл
              --zapret-root <путь> Корень установки Zapret (ищется сам)

              Пробы идут разом, Esc прерывает и показывает набранное.

              Отчёт из --out можно приложить к issue целиком: там и шапка
              с обстановкой замера, и причины отказов, которые в пересказе
              теряются первыми.
