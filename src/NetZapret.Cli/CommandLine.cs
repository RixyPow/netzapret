...
internal sealed class CommandLine
{
    ...
    /// <summary>
    /// Объявляет опции, которые команда понимает, не читая их.
    /// </summary>
    /// <remarks>
    /// Нужно там, где команда возвращается рано и до чтения части опций дело
    /// не доходит: холостой прогон печатает разбор и выходит, а <c>--no-proxy</c>
    /// прочитался бы только при настоящем запуске. Без объявления команда
    /// сообщала бы, что не знает опции, зная её прекрасно, — а ложное
    /// предупреждение хуже отсутствующего, оно приучает не читать их вовсе.
    /// </remarks>
    public void Declare(params string[] keys)
    {
