...
internal static class BlockCheckCommand
{
    ...
    private static readonly Dictionary<string, string> FalseAlarmProne =
        new(StringComparer.OrdinalIgnoreCase)
        {
            ...
            // updates.discord.com отсюда убран, и это не смягчение, а признание.
            // Оговорка гласила «обновления идут своим каналом, не через 443»,
            // и звучала убедительно — а на деле имя падало оттого, что один
            // из пяти его адресов закрыт и съедал весь срок пробы. Теперь
            // адреса опрашиваются по одному, имя проходит проверку, и держать
            // оговорку значило бы глушить его настоящий отказ, когда он придёт.
