...
internal static class MenuCommand
{
    ...
    private static void PinPart(ServiceRouting.PartStatus part, UserRulesFile file, MatchKind kind)
    {
        ...
        try
        {
            ...
            // Запись удалась — но это ещё не значит, что она есть. Kaspersky
            // считает изменённый hosts признаком заражения и возвращает файл
            // к своему умолчанию; ошибки при этом нет ни одной, а блока
            // через мгновение нет тоже. Сообщать об успехе того, чего больше
            // не существует, — худшее, что средство настройки может сделать.
            if (!HostsEditor.BlockSurvived())
            {
                var who = HostsEditor.WhoReplaced() ?? "защитник";

                Message(
                    $"Записал — и записи уже нет: {who} вернул hosts к своему умолчанию. " +
                    "Пин на этой машине не живёт. Либо внесите файл hosts в доверенные " +
                    "в настройках защитника, либо ведите эти имена маршрутом.",
                    ConsoleColor.Red);
