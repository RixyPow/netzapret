...
public sealed record ProbeOptions
{
    ...
    /// <summary>
    /// Сколько ждать, пока поднимется движок проверки.
    /// </summary>
    /// <remarks>
    /// Поднимается он за доли секунды: ни TUN, ни маршрутов, ни правил —
    /// один инбаунд и один исходящий. Восемь секунд были запасом на случай
    /// занятого диска, но платил за этот запас каждый мёртвый сервер.
    /// </remarks>
    public TimeSpan StartupTimeout { get; init; } = TimeSpan.FromSeconds(4);

    /// <summary>
    /// Сколько ждать ответа по одному адресу проверки связности.
    /// </summary>
    /// <remarks>
    /// Адреса нарочно лёгкие и отдают пустой ответ. Сервер, которому нужно
    /// больше трёх секунд на такой запрос, для работы всё равно непригоден:
    /// это не медленный сервер, а недоступный.
    /// </remarks>
    public TimeSpan RequestTimeout { get; init; } = TimeSpan.FromSeconds(3);

    /// <summary>
    /// Общий бюджет времени на один сервер — потолок проверки.
    /// </summary>
    /// <remarks>
    /// <para>
    /// Обязателен: без него мёртвый сервер съедает таймаут за каждый адрес
    /// подряд, и проверка занимает минуты вместо десятков секунд.
    /// </para>
    /// <para>
    /// Было четырнадцать секунд. На подписке в три десятка серверов, где
    /// половина молчит, это минута с лишним ожидания — и всё это время окно
    /// выглядело зависшим. Восьми хватает: запуск движка плюс два адреса
    /// подряд по три секунды.
    /// </para>
    /// </remarks>
    ...
    /// полтора десятка движков разом. Восемь — компромисс: тридцать четыре
    /// сервера укладываются в полминуты даже когда молчат все.
    /// </remarks>
