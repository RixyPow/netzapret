...
public sealed class SingBoxConfigCompiler
{
        ...
        SingBoxOptions options)
    {
        ...
        if (options.DeadServerTags.Count > 0)
        {
            var alive = eligible
                .Where(s => !options.DeadServerTags.Contains(tags[s]))
                .ToList();

            // И снова не до пустоты. Подписка бывает мёртвой целиком —
            // тогда пусть движок перебирает трупы, это всё же лучше, чем
            // конфиг, который не читается.
            if (alive.Count > 0)
                eligible = alive;
        }
