...
public sealed record WarpAccount
{
    ...
    /// <summary>
    /// Выходы WARP, которые нужно подмешать к серверам подписки.
    /// </summary>
    /// <remarks>
    /// <para>
    ...
    /// в списке подписок занимал место основной вместо того, чтобы её
    /// страховать — выбор его выхода отключал рабочий VPN целиком.
    /// </para>
    /// <para>
    /// Выходов два, и различаются они транспортом: WireGuard на ключах,
    /// которые заводим мы, и MASQUE поверх QUIC, где запись движок заводит
    /// себе сам. Второй возвращается всегда, даже без ключей, — ему от нас
    /// ничего не нужно.
    /// </para>
    /// <para>
    /// Который из двух пройдёт, заранее не известно и зависит от оператора.
    /// Выбирать за человека не беремся: автоподбор опрашивает обоих.
    /// </para>
    /// </remarks>
    public static IReadOnlyList<ProxyServer> Exits()
    {
