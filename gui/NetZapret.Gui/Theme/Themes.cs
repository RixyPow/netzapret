...
/// </para>
/// <para>
/// Применяется сразу, и ради этого вся разметка ссылается на палитру через
/// <c>DynamicResource</c>. Со <c>StaticResource</c> подмена словаря
/// не действовала на то, что уже нарисовано: ссылка разрешается один раз
/// при загрузке, и открытые разделы оставались в прежних цветах до тех пор,
/// пока их не пересоздадут. Снаружи это выглядело так, будто тема меняется
/// через раз.
/// </para>
/// <para>
/// Цена у динамической ссылки есть — поиск по дереву ресурсов при каждом
/// обращении, — но она платится один раз на элемент при отрисовке, а речь
/// о полутысяче кистей в неподвижном окне.
/// </para>
/// </remarks>
