...
public static class SiteIcons
{
    ...
    /// <summary>
    /// Уже загруженный значок — из памяти, а если там пусто, то с диска.
    /// </summary>
    /// <remarks>
    /// <para>
    /// Раздел с маршрутами пересоздаётся при каждом заходе, и без памяти он
    /// каждый раз начинал бы с букв, а значки проступали бы по одному
    /// заново — при живом кэше. Мигание на ровном месте: данные есть,
    /// а показываются так, будто их нет.
    /// </para>
    /// <para>
    /// Диск добавлен потому, что после перезапуска памяти нет вовсе, и
    /// то же мигание возвращалось при полном кэше на диске: восемьдесят пять
    /// файлов лежат, а список открывается буквами и наливается значками
    /// секунду спустя. Снаружи это читается как «значки не сохраняются».
    /// </para>
    /// <para>
    /// Чтение по одному имени, а не обходом папки: имя файла получается
    /// из хоста с заменой недопустимых знаков, и обратно оно не выводится —
    /// <c>104.244.42.0_24</c> это и подсеть, и имя с подчёркиванием.
    /// Прочитанное запоминается, так что на каждое имя приходится одно
    /// обращение к диску за весь сеанс.
    /// </para>
    /// </remarks>
    public static BitmapImage? Cached(string host)
    {
        ...
        {
            // Испорченный файл кэша не повод не показать раздел: значок
            // просто уступит место букве, а дозагрузка попробует ещё раз.
            return null;
        }

        // В память кладём только найденное. Отсутствие файла означает,
        // что значок ещё не спрашивали, — и дозагрузка должна это сделать,
        // а не считать имя уже обслуженным.
        if (image is null)
            return null;
