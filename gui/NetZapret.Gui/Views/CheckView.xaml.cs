...
public partial class CheckView : UserControl
{
    ...
    /// <summary>Полная глубина: по три имени с каждой части вместо одного.</summary>
    ...
    private void ShowScopes()
    {
        ...
        try
        {
            ...
            // по три, точечная — одна цель целиком. Прежде под этими двумя
    ...
    private async void OnRun(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            ...
            Say(_target is not null
                ...
                : _deep
                    ? $"Полная проверка: {targets.Count} имён, по три с каждой части, "
        ...
        bool deep)
    {
        ...
        // Одно имя с части — быстрая; три — полная. Второе имя и отделяет
        // случайную неудачу одного хоста от закрытой части целиком: одно имя
        // ошибается само по себе, два подряд — уже нет. Третье — с 26.09:
        // у Claude третьей строкой стоит claudeusercontent.com, который
        // ломается сам по себе (владелец выбрал слить его в одну часть
        // с claude.ai и расширить проверку, а не держать две части с одним
        // маршрутом). Для одного сервиса берётся четыре: там время есть,
        // а подробность и есть смысл выбора.
        int perPart = only is not null ? 4 : deep ? 3 : 1;
